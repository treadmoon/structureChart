...
3. **开放安全组端口**:
   ...
   - 编辑入站规则（Inbound Rules），添加一条规则允许 `TCP 端口 51981` 从 `0.0.0.0/0` 访问。
...
4. **访问**: 当流水线出现绿色勾选（成功），您即可在浏览器中访问您的 `http://[EC2_PUBLIC_IP]:51981` 查看在线版的 `structure-chart` 演示了！
...
docker run -d -p 51981:80 --name structure-chart structure-chart
```
